...
def preprocess_device_action(action: dict[str, Any], teleop_device) -> torch.Tensor:
    if action.get("so101_leader") is not None:
    ...
    elif action.get("so101_joint_keyboard") is not None:
        processed_action = torch.zeros(teleop_device.env.num_envs, 6, device=teleop_device.env.device)
        processed_action[0, :] = torch.tensor(action["joint_state"], device=teleop_device.env.device, dtype=torch.float32)
    elif action.get("keyboard") is not None or action.get("gamepad") is not None:
        action_dim = teleop_device.env.action_manager.total_action_dim
        processed_action = torch.zeros(teleop_device.env.num_envs, action_dim, device=teleop_device.env.device)
        processed_action[:, :] = action["joint_state"][:action_dim]
        ...
        left_action = convert_action_from_so101_leader(
            action["joint_state"]["left_arm"], action["motor_limits"]["left_arm"], teleop_device
        )
        right_action = convert_action_from_so101_leader(
            action["joint_state"]["right_arm"], action["motor_limits"]["right_arm"], teleop_device
        )
        processed_action[:, :6] = left_action
        processed_action[:, 6:] = right_action
    elif action.get("bi_keyboard") is not None:
        processed_action = torch.zeros(teleop_device.env.num_envs, 12, device=teleop_device.env.device)
        processed_action[:, :6] = torch.tensor(action["joint_state"]["left_arm"], device=teleop_device.env.device, dtype=torch.float32)
        processed_action[:, 6:] = torch.tensor(action["joint_state"]["right_arm"], device=teleop_device.env.device, dtype=torch.float32)
